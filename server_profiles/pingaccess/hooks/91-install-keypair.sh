...
keypairId=$(curl -X POST --silent --basic -u Administrator:${PA_ADMIN_PASSWORD} --header 'Content-Type: application/json' --header 'Accept: application/json' --header 'X-XSRF-Header: PingAccess' --data '@install-cert.txt' https://localhost:9000/pa-admin-api/v3/keyPairs/import --insecure | jq .id)
...
then
  ...
  configureHttpsResponse=$(curl -X PUT --silent --basic -u Administrator:${PA_ADMIN_PASSWORD} --header 'Content-Type: application/json' --header 'Accept: application/json' --header 'X-XSRF-Header: PingAccess' --data '@configure-https.txt' https://localhost:9000/pa-admin-api/v3/httpsListeners/${httpsEngineId} --insecure)

  echo "Keypair assigned to Engine HTTPS listener: $httpsEngineId"

  engineName="ADMIN"
  echo $("generate_httpslistener_data") > configure-https.txt
  configureHttpsResponse=$(curl -X PUT --silent --basic -u Administrator:${PA_ADMIN_PASSWORD} --header 'Content-Type: application/json' --header 'Accept: application/json' --header 'X-XSRF-Header: PingAccess' --data '@configure-https.txt' https://localhost:9000/pa-admin-api/v3/httpsListeners/${adminHttpsEngineId} --insecure)
