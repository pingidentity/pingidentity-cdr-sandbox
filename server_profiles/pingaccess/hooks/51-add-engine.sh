...
if [[ ! -z "${OPERATIONAL_MODE}" && "${OPERATIONAL_MODE}" = "CLUSTERED_ENGINE" ]]; then
    ...
    while true; do  
    	...
    	keypairid=$(curl -ss --silent -k -u Administrator:"${INITIAL_ADMIN_PASSWORD}" -H "X-Xsrf-Header: PingAccess" https://${pahost}:9000/pa-admin-api/v3/httpsListeners | jq '.items[] | select(.name=="CONFIG QUERY") | .keyPairId')
    	echo "KeyPairId:"${keypairid}
    	
    	if [ -z "$keypairid" ]
    	then
    		echo "Have not found keypairid"
    		sleep 3
    	else
    		echo "Found keypairid"
    		break
    	fi
	done
