...
if [[ ! -z "${OPERATIONAL_MODE}" && "${OPERATIONAL_MODE}" = "CLUSTERED_ENGINE" ]]; then
    ...
    echo "Testing whether we can actually receive a key pair id..."
	while true; do
		echo "Test retrieving Key Pair ID from administration API..."
		curl -ss --silent -o /dev/null -k -v -k -u Administrator:"${INITIAL_ADMIN_PASSWORD}" -H "X-Xsrf-Header: PingAccess" https://${pahost}:9000/pa-admin-api/v3/httpsListeners | jq '.items[] | select(.name=="CONFIG QUERY") | .keyPairId'
		if ! test $? -eq 0 ; then
			echo "Unable to retrieve Key Pair ID. Waiting..."
			sleep 3
		else
			echo "Key Pair ID received, continuing..."
			break
		fi
	done
